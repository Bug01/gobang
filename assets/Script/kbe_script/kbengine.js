...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.reset = function()
	{
		...
		KBEngine.app.clientVersion = "2.5.1";
	...
	*/
	...
	this.Client_onLoginSuccessfully = function(args)
	{
		...
		KBEngine.INFO_MSG("KBEngineApp::Client_onLoginSuccessfully: accountName(" + accountName + "), addr(" + KBEngine.app.baseappIp + ":" + 
				KBEngine.app.baseappTCPPort + "|" + KBEngine.app.baseappUDPPort + "), datas(" + KBEngine.app.serverdatas.length + ")!");
	...
	this.onUpdatePropertys_ = function(eid, stream)
	{
		...
		while(stream.length() > 0)
		{
			...
			if(currModule.usePropertyDescrAlias){
				var componentID = stream.readInt8();
				utype = stream.readUint8();
			}
			else{
				var componentID = stream.readUint16();
	...
	this.Client_onUpdateBaseDir = function(stream)
	{
